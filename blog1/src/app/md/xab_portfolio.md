...
## 1. 전체적인 아키텍처 (Overall Architecture)
```mermaid
graph TD
...
```
- 초기 로딩 속도를 개선하기 위해 사용자가 볼 가능성이 높은 상위 10개의 데이터만 우선적으로 로드하고 나머지는 스크롤 시점에 가져오는 Lazy Loading 전략을 수립했습니다.
- `useInfiniteQuery`로 데이터를 페이지 단위로 분할하고, `Intersection Observer` API를 활용해 뷰포트 하단의 투명 트리거 요소가 10% 이상 노출될 때만 다음 페이지를 호출하도록 구현했습니다.
- 불필요한 리소스 낭비 없이 끊김 없는 스크롤 경험을 제공했습니다.
...
- `postgres_changes` 이벤트를 구독하여 댓글 추가나 투표 발생 시 서버가 즉시 알림을 보내도록 설계했습니다.
- React Query의 캐시 무효화(Invalidation) 로직과 연동하여 사용자의 개입 없이 항상 최신 상태("Live")를 유지하는 피드를 구현했습니다.
...
## 4. 배운 점 (Insights)
- 이 프로젝트를 통해 '기술적 구현'과 '사용자 경험' 사이의 연결고리를 깊게 고민하게 되었습니다.
- 단순히 코드를 줄이는 것이 아니라 사용자가 느끼는 LCP, CLS 등 Web Vitals 지표를 개선하는 것이 진짜 최적화임을 배웠습니다.
- 병렬 라우트나 실시간 데이터 동기화 같은 고급 패턴을 도입할 때 공식 문서의 설계 의도를 파악하는 것이 가장 빠르고 정확한 길임을 확신하게 되었습니다.
